...
float x, y, z;           // [mm] Coordinates of hedgehog (X,Y,Z) 
...
//Error Calculations//
ez = zDes - z;        //Error
ezD1 = z - zOld;   // Error Derivative
eiz += ez;        // Error Integral

//Control Law//
u1 = kpz*ez + kiz*eiz + kdz*ezD1; //+ kdz*ezD1 + kiz*eiz; // [N]

//ux = kpx*ex + kix*eix + kdx*exD1; //+ kdz*ezD1 + kiz*eiz; // [N]
//ux = kp
//uy =
//Serial.println(z);
//Serial.println(u1);
...
void smc(){
  ...
  //pitchDes=atan(-uy/sqrt(ux*ux+(uz+g)*(uz+g)));
  //rollDes=atan(ux/(uz+g));
  pitchDes=0;
  rollDes=0;
  yawDes=yawDesD1*dt;
