///////////////////////////////////////////////////////////////////////////////////////
//Sliding Mode Quadcopter Flight Controller V4 (SMCattitudeIR.ino)
///////////////////////////////////////////////////////////////////////////////////////
...
float mag_offsets[3]            = { 10.01, -48.23, 31.22 };

// Soft iron error compensation matrix
float mag_softiron_matrix[3][3] = { {  0.987,  -0.039,  -0.014 },
                                    {  -0.030,  0.997, -0.034 },
                                    {  -0.014, -0.034,  1.019 } };

float mag_field_strength        = 39.29;
...
float rollDCal, pitchDCal, yawDCal, yawCal;

float pitchCal =-0.017953072; //[rad]
float rollCal = -0.052949127; //[rad]
...
//Gyroscope//
float gyroYaw;   //Euler Angle
float accPitch, accRoll, accPitchCal, accRollCal;
long accX, accY, accZ, accA;
int calcInt;

//Sliding Mode Controller//
float pitch, roll, yaw;             //Euler Angles
float pitchD, rollD, yawD;       //Euler Angular Rates
float ePitch, eRoll, eYaw;          //Error
float epitchD, erollD, eyawD;    //Error Derivative
...
void loop(){
  ...
  pitch -= roll * sin(yawD*dt);                  //If the IMU has yawed transfer the roll angle to the pitch angle.
  roll += pitch * sin(yawD*dt);                  //If the IMU has yawed transfer the pitch angle to the roll angle.
  ...
  epitchD = pitchDesD1 - pitchD;
  erollD = rollDesD1 - rollD;  
  eyawD = yawDesD1 - yawD;

  //Sliding Surfaces//
  sPitch = cPitch*ePitch + epitchD;
  sRoll = cRoll*eRoll + erollD;
  sYaw = cYaw*eYaw + eyawD;
  ...
  u2 = ((kPitch*sPitch) + (cPitch*epitchD) + pitchDesD2  /* + (a2*rollD*wr)*/ - (a1*rollD*yawD))/b1;   // [Nm] Pitch Torque
  u3 = ((kRoll*sRoll) + (cRoll*erollD) + rollDesD2 /*- (a4*pitchD*wr)*/ - (a3*pitchD*yawD))/b2;        // [Nm] Roll Torque
  u4 = ((kYaw*sYaw) + (cYaw*eyawD) + yawDesD2 - (a5*pitchD*rollD))/b3;                                 // [Nm] Yaw Torque
...
void gyro_signalen(){
  ...
  accZ = accel_event.acceleration.x;
  accA = sqrt((accX*accX)+(accY*accY)+(accZ*accZ));

  //Accelerometer Angles [rad]//
  accPitch = asin(accY/accA);
  accRoll = -asin(accX/accA);

  
 
  //Gyro Rates [rad/s]//
  pitchD = gyro_event.gyro.y - pitchDCal;
  rollD = gyro_event.gyro.x - rollDCal;
  yawD = gyro_event.gyro.z - yawDCal;
  ...
  //Filtered Yaw//
  filter.update(rollD/deg2rad, pitchD/deg2rad, yawD/deg2rad, accX, accY, accZ, mx, my, mz);
...
/** Debug Section
  ...
  Serial.println("epitchD");
  Serial.println(epitchD);
  Serial.println("erollD");
  Serial.println(erollD);
  Serial.println("eyawD");
  Serial.println(eyawD);
  ...
  Serial.println("GYRO");
  Serial.println(roll/deg2rad);
  Serial.println(accRoll/deg2rad);
