...
float mag_offsets[3]            = { 16.32F, -39.28F, 41.07F };

// Soft iron error compensation matrix
float mag_softiron_matrix[3][3] = { {  0.982, -0.029,  0.013 },
                                    {  -0.029,  0.977, -0.034 },
                                    {  0.013, -0.034,  1.045 } };

float mag_field_strength        = 37.36F;

// Offsets applied to compensate for gyro zero-drift error for x/y/z
float gyro_zero_offsets[3]      = { 0.000598052F, -0.000124524F, 0.002965979F };
...
float yawCal2 =-1.15; //[rad] Yaw Angle Calibration
...
float accXcal1, accYcal1, accZcal1;
float accXcal2, accYcal2, accZcal2;
float gxCal1, gyCal1, gzCal1;
float gxCal2, gyCal2, gzCal2;
float gx, gy, gz;

//Sliding Mode Controller//
float pitch, roll, yaw, yawCal1;       //Euler Angles

float pitchD1, rollD1, yawD1;       //Euler Angular Rates
float ePitch, eRoll, eYaw, eYawOld;          //Error
...
void setup(){
  ...
  gpsCal[2] /= 2000.0F;  // y
  gpsCal[3] /= 2000.0F;  // z
  accXcal2 = accXcal1/2000.0;
  accYcal2 = accYcal1/2000.0;
  accZcal2 = accZcal1/2000.0;
  gxCal2 = gxCal1/2000.0;
  gyCal2 = gyCal1/2000.0;
  gzCal2 = gzCal1/2000.0;
  accZcal2-=g;
...
void gyro_signalen(){
  ...
  gx = gyro_event.gyro.x - gxCal1;
  gy = gyro_event.gyro.y - gyCal1;
  gz = gyro_event.gyro.z - gzCal1;

  //Accelerometer Values//
  accX = accel_event.acceleration.x - accXcal2;
  accY = accel_event.acceleration.y - accYcal2;
  accZ = accel_event.acceleration.z - accZcal2;
  ...
  //Angles [rad]//
  yaw = (-filter.getYaw())*deg2rad - yawCal2;
  if (yaw<-PI) yaw+=2*PI;
}
